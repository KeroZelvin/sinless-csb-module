...
1. **SinlessCSB `TableResult#text` deprecation (v15 break).** `scripts/api/table-to-tile.js:69` reads `result.text`; deprecated since v13, **removed in v15**. Fires on every chase/table-to-tile draw. Low severity now; must be migrated to `result.name`/`result.description` before Foundry v15. (Not fixed in Phase B — module code change, candidate for Phase C.)
