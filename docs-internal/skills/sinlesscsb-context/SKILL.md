...
2) Sync recent Foundry JSON exports from Downloads into the repo template folder:
   - Run:
     `powershell -ExecutionPolicy Bypass -File docs-internal/skills/sinlesscsb-context/scripts/sync-recent-foundry-exports.ps1 -LookbackDays 2 -Apply`
...
4) If the task involves routing Create/Import JSON macros or locating content by folder,
   ...
   - Run:
     `powershell -ExecutionPolicy Bypass -File docs-internal/skills/sinlesscsb-context/scripts/select-latest-folder-tree.ps1 -PruneTracked`
...
6) If the task is CSB-specific, consult the CSB wiki:
